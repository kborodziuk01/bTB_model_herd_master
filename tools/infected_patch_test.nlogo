patches-own [c]  ; Declare the patch variable "c" in patches-own
...
to go
  ...
  ; Initialize the variable "c" for all patches
  ask patches [ set c 0 ]

  ; Iterate over the list, adding 1 to "c" for each patch
  foreach patch-list [current-patch ->
    ask current-patch [
      set c c + 1
    ]
  ]

  ; Scale the color of patches based on the "c" value
  ask patches [
    set pcolor scale-color green c 0 cutoff
  ]
